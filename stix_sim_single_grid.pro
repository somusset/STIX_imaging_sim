FUNCTION stix_sim_transmission_patterns, collimator_param, xfront, yfront, incidence_angles, pass_grid_1=pass_grid_1, pass_grid_2=pass_grid_2
  ...
  ;   
  ; :description:
  ;   This function returns the transmission value through a pair of STIX grids, for a point-source at infinite distance at a given incidence angle, 
...
FUNCTION stix_sim_single_grid, incidence_angles, grid_parameters, npoints=npoints, n_counts=n_counts, loud=loud, plot_interm=plot_interm, $ 
...
;   STIX imaging system simple simulation
; :description:
;   This function simulate output of a subcollimator of STIX (a pair or grids and a detector) to a point source located at infinite distance, arriving at a certain incidence angle.
...
; 
; :call:
;   stix_sim_transmission_patterns
; 
; :example:
;   stix_sim_single_grid, [0.001, 0.0], [0.320259, 0.330680, 150.52, 149.462], /loud
; 
; :history:
;   2019-01-10: Sophie Musset (University of Minnesota): initial release
; 
...
;-
  ...
  D1 = 550. + 0.4   ; mm distance between grids + width of grid
  D2 = 47. + 0.4    ; mm distance between rear grid and detector + width of grid
  h_det = 9.2       ; mm detector height
  w_det = 8.8       ; mm detector width
  h_fgrid = 20.     ; mm front grid height
  w_fgrid = 22.     ; mm front grid width
  h_rgrid = 13.     ; mm rear grid height
  w_rgrid = 13.     ; mm rear grid width
  ...
  FOR k = 0, npoints DO BEGIN                                                         ; go through all the x positions
    ...
    FOR i = 0, npoints DO BEGIN                                                       ; go through the y positions
      ...
      pos_ondet = stix_sim_transmission_patterns(collimator_param, xfront[k], yfront[i], incidence_angles, pass_grid_1=pass_grid_1, pass_grid_2=pass_grid_2)
      frontgrid_transmission[k,i] = pass_grid_1
      reargrid_transmission[k,i] = pass_grid_2

 ;     IF pass_grid_1 EQ 1 AND pass_grid_2 EQ 1 THEN BEGIN
 ;       moire_x.add, pos[0]
 ;       moire_y.add, pos[1]
 ;       IF pos[0] LT b_grid/2. AND pos[0] GT -1*b_grid/2. AND pos[1] LT a_grid/2. AND pos[1] GT -1*a_grid/2. THEN BEGIN
 ;         det_x.add, pos[0]
 ;         det_y.add, pos[1]
 ;       ENDIF
 ;     enDIF
 ...
 ;     yf[i] = pos[1]
  ...
  r_pixel_val = fltarr(4,2)
  r_detector_pixelise = moire_on_detector
  FOR k=0,1 DO BEGIN ; k is on the x axis {horizontal}
    FOR i =0,3 DO BEGIN ; i is on the y axis {vertical}
      r_pix = where(xdet[arrayx] GT i*w_det/4. - w_det/2. AND xdet[arrayx] LT (i+1)*w_det/4. - w_det/2. AND ydet[arrayy] GT k*h_det/2. - h_det/2. AND ydet[arrayy] LT (k+1)*h_det/2. - h_det/2.)
      r_pixel_val[i,k] = N_elements(r_pix)
      IF loud EQ 1 THEN print, 'pixel ',i,N_elements(r_pix),' counts/s'
      r_detector_pixelise[i*nx/4.:(i+1)*nx/4.-1, k*ny/2.:(k+1)*ny/2.-1] = r_pixel_val[i,k]
    ENDFOR
  ENDFOR
